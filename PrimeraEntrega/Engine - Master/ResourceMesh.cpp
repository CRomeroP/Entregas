...
// ---------------------------------------------------------
ResourceMesh::ResourceMesh(unsigned long long uid) : Resource(uid, Resource::Type::mesh)
{
	...
	if (GetExportedFile() != nullptr)
	{
		...
		if ((attribs & ATTRIB_BONES) != 0)
		{
			...
			for (unsigned int i = 0; i < num_bones; ++i)
			{
				...
				for (unsigned int j = 0; j < bones[i].num_weights; ++j)
				{
...
// ---------------------------------------------------------
unsigned long long ResourceMesh::Import(const aiMesh* mesh, const char* source_file)
{
...
unsigned long long ResourceMesh::LoadSphere(const char* sphere_name, float size, unsigned slices, unsigned stacks)
{
	...
	if (mesh)
	{
		...
		unsigned long long uid = Generate(sphere_name, mesh);
...
unsigned long long ResourceMesh::LoadCylinder(const char* cylinder_name, float height, float radius, unsigned slices, unsigned stacks)
{
	...
	if (mesh)
	{
		...
		for (unsigned int i = 0; i < unsigned int(mesh->npoints); ++i)
		{
			std::swap(mesh->tcoords[i * 2], mesh->tcoords[i * 2 + 1]);
			mesh->tcoords[i * 2] *= 2.0f;
		}

		unsigned long long uid = Generate(cylinder_name, mesh);
...
unsigned long long ResourceMesh::LoadTorus(const char* torus_name, float inner_r, float outer_r, unsigned slices, unsigned stacks)
{
	...
	if (mesh)
	{
		...
		unsigned long long uid = Generate(torus_name, mesh);
...
unsigned long long ResourceMesh::LoadCube(const char* cube_name, float size)
{
	...
	if (mesh)
	{
		...
		unsigned long long uid = Generate(cube_name, mesh);
...
unsigned long long  ResourceMesh::LoadPlane(const char* plane_name, float width, float height, unsigned slices, unsigned stacks)
{
	...
	if (mesh)
	{
		...
		unsigned long long uid = Generate(plane_name, mesh);
...
unsigned long long ResourceMesh::Generate(const char* shape_name, par_shapes_mesh* shape)
{
	ResourceMesh* m = static_cast<ResourceMesh*>(App->resources->CreateNewResource(Resource::mesh));
...
void ResourceMesh::GenerateCPUBuffers(par_shapes_mesh* shape)
{
	...
	for (unsigned int i = 0; i < unsigned int(shape->ntriangles) * 3; ++i)
	{
