...
unsigned int Config::GetUInt(const char * field, unsigned int default, int index) const
{
	JSON_Value* value = FindValue(field, index);

	if (value && json_value_get_type(value) == JSONNumber)
		return (unsigned int)json_value_get_number(value);

	return default;
}

unsigned long long Config::GetUID(const char * field, unsigned long long default, int index) const
{
	JSON_Value* value = FindValue(field, index);

	if (value && json_value_get_type(value) == JSONNumber)
		return (unsigned long long)json_value_get_number(value);
...
bool Config::AddUInt(const char * field, unsigned int value)
{
	return json_object_set_number(root, field, (double)value) == JSONSuccess;
}

bool Config::AddUID(const char * field, unsigned long long value)
{
	return json_object_set_number(root, field, (double)value) == JSONSuccess;
}
...
bool Config::AddArrayUInt(const char * field, const unsigned int * values, int size)
{
	if (values != nullptr && size > 0)
	{
		JSON_Value* va = json_value_init_array();
...
bool Config::AddArrayUID(const char * field, const unsigned long long * values, int size)
{
	if (values != nullptr && size > 0)
	{
		JSON_Value* va = json_value_init_array();
